  ...
  for ( var f = 0; f < bufs.length; f++ ) {
    ...
    if ( flag != 1 && flag != 2 && flag != 3 ) {
      ...
      for (var a = 0; a < argc; a++) {
        ...
        var len = ((bufs[f][offset++] << 8) + (bufs[f][offset++])) >>> 0; 		// equivalent to readUInt16BE.
    ...
    //if ( flag < 4 ) {
      //sequence_number = ((bufs[f][offset++] << 24) + (bufs[f][offset++] << 16) + (bufs[f][offset++] << 8) + (bufs[f][offset++])) >>> 0;   // equivalent to readUInt32BE.
